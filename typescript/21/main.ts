// I had so much trouble understanding the prompt for part 1. Even assuming
// every possible allergen is shown at least once, it really doesn't seem like
// the input has enough information to figure out which ingredients definitely
...
function getRuledOutAllergens(foods: Food[]): Map<Ingredient, Set<Allergen>> {
    const allIngredients = new Set(foods.flatMap((food) => food.ingredients))
    const ruledOutAllergens = new Map<Ingredient, Set<Allergen>>()
    for (const food of foods) {
        const ingredientsInThisFood = new Set(food.ingredients)
        const ingredientsNotInThisFood = Array.from(allIngredients).filter(
            (ingredient) => !ingredientsInThisFood.has(ingredient),
        )
        for (const ingredient of ingredientsNotInThisFood) {
            const alreadyRuledOutAllergens = ruledOutAllergens.get(ingredient)
            if (alreadyRuledOutAllergens === undefined) {
                ruledOutAllergens.set(ingredient, new Set(food.allergens))
            } else {
                for (const allergen of food.allergens) {
                    alreadyRuledOutAllergens.add(allergen)
                }
                ruledOutAllergens.set(ingredient, alreadyRuledOutAllergens)
            }
        }
    }
    return ruledOutAllergens
}

function reducePossibleAllergens(
    possibleAllergens: Map<Ingredient, Set<Allergen>>,
): Map<Ingredient, Allergen> {
    const knownAllergens = new Map()
    while (possibleAllergens.size > 0) {
        const definiteAllergens = Array.from(possibleAllergens)
            .filter(([_, allergens]) => allergens.size === 1)
            .map(
                ([ingredient, oneAllergen]) =>
                    [ingredient, Array.from(oneAllergen)[0]] as const,
            )
        for (const [knownIngredient, knownAllergen] of definiteAllergens) {
            knownAllergens.set(knownIngredient, knownAllergen)
            possibleAllergens.delete(knownIngredient)
            for (const [
                _,
                possibleAllergensForIngredient,
            ] of possibleAllergens) {
                possibleAllergensForIngredient.delete(knownAllergen)
            }
        }
    }
...
function parse(input: string): Food {
    const [ingredients, allergens] = input
        .slice(0, -1)
        .split('(contains')
...
const foods = inputFoods.map(parse)
const allAllergens = new Set(foods.flatMap((food) => food.allergens))
const ruledOutAllergens = getRuledOutAllergens(foods)

const ingredientsWhichContainNoAllergens = new Set(
    Array.from(ruledOutAllergens)
        .filter(([_, ruledOutAllergens]) => {
            return ruledOutAllergens.size === allAllergens.size
        })
        .map(([ingredient, _]) => ingredient),
)

const howManyTimesNonAllergenIngredientsAppear = Array.from(
    ingredientsWhichContainNoAllergens,
)
    .flatMap((nonAllergenIngredient) =>
        foods.map((food) => {
            return food.ingredients.filter(
                (ingredient) => ingredient === nonAllergenIngredient,
            ).length
        }),
    )
    .reduce((a, b) => a + b, 0)
...
console.log(
    'How many times non-allergen ingredients appear:',
    howManyTimesNonAllergenIngredientsAppear,
)

const possibleAllergens: Map<Ingredient, Set<Allergen>> = new Map(
    Array.from(ruledOutAllergens.entries())
        .filter(([ingredient, _]) => {
            return !ingredientsWhichContainNoAllergens.has(ingredient)
        })
        .map(([ingredient, allergensNotInThisIngredient]) => {
            const allergensThatMightBeInThisIngredient = new Set(
                Array.from(allAllergens).filter((allergen) => {
                    return !allergensNotInThisIngredient.has(allergen)
                }),
            )
            return [ingredient, allergensThatMightBeInThisIngredient]
        }),
)

const determinedAllergens = reducePossibleAllergens(possibleAllergens)
const canonicalDangerousIngredientList = Array.from(
    determinedAllergens.entries(),
)
    .sort(([_ingredient1, allergen1], [_ingredient2, allergen2]) => {
        return allergen1 < allergen2 ? -1 : +1
    })
    .map(([ingredient, _]) => ingredient)
    .join(',')
