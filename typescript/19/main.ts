...
function createRegExp(inputRules: Rules): RegExp {
    const rules = Object.values(inputRules)
    ...
    while (/\d/g.test(rules[0])) {
        for (const index in rules) {
            ...
            rules[index] = reducedRule
        }
    }

    return new RegExp(`^${rules[0].replace(/ /g, '')}$`)
}
...
const regExp1 = createRegExp(input.rules)
const matchingMessages1 = input.messages.filter((message) =>
    regExp1.test(message),
)
console.log(
    'How many messages match rules (with mistakes)?',
    matchingMessages1.length,
)

// Part 2 has some "fixes" for rules. I manually altered both of them so I
// didn't have to adjust createRegExp.

// 8: 42 | 42 8
// This one is just rule 42 repeated one or more times.
input.rules[8] = '42+'

// 11: 42 31 | 42 11 31
// This one is more interesting. It's non-regular, with rule 42 and 31 both
// repeating the *same* number of times (at least once).
...
const matchingMessages2 = input.messages.filter((message) =>
    regExp2.test(message),
)
console.log(
    'How many messages match rules (with fixes)?',
    matchingMessages2.length,
)
