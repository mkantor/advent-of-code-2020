...
type AccumulatorValue = number
class VM {
    public accumulator: AccumulatorValue = 0
    public instructionPointer: InstructionIndex = 0
    public instructionHistory: InstructionIndex[] = []

    constructor(public instructions: Instruction[]) {}

    tick(): boolean {
        this.instructionHistory.push(this.instructionPointer)
...
function fixAndGetFinalAccumulator(vm: VM): AccumulatorValue | undefined {
    if (detectLoopAndGetAccumulator(vm) === undefined) {
        ...
        for (const pointer of encounteredNopsAndJmps) {
            ...
            // Dumb deep clone.
            const adjustedInstructions = JSON.parse(
                JSON.stringify(vm.instructions),
            )
            adjustedInstructions[pointer][0] = newOperation
...
const accumulatorValueBeforeLoop = detectLoopAndGetAccumulator(
    new VM(instructions),
)
console.log('Accumulator value before loop:', accumulatorValueBeforeLoop)

const finalAccumulatorValueAfterFix = fixAndGetFinalAccumulator(
    new VM(instructions),
)
console.log(
    'Final accumulator value after fixing the corrupted instruction:',
    finalAccumulatorValueAfterFix,
)
