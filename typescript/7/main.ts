// Maps container bag to acceptable contents (a tuple of count + bag).
type Rules = {
...
function findContainerBags(targetBag: string, rules: Rules): Set<string> {
    const findContainerBagsInner = (
        targetBag: string,
        rules: Rules,
    ): string[] => {
        let directContainers = Object.entries(rules)
            .filter(([_, value]) => {
                return value.some(([_, bagType]) => bagType === targetBag)
            })
            .map(([bag, _]) => bag)

        const containersOfContainers = directContainers.flatMap((container) =>
            findContainerBagsInner(container, rules),
        )
        return [...directContainers, ...containersOfContainers]
    }

    // We want unique bag types.
    return new Set(findContainerBagsInner(targetBag, rules))
}

function findBagContents(
    targetBag: string,
    rules: Rules,
    multiplier: number = 1,
): [number, string][] {
...
const rules = parseRules(ruleDescriptions)

const myBag = 'shiny gold bag'

const containersForMyBag = findContainerBags(myBag, rules)
console.log(
    `Number of bags that can contain ${myBag}:`,
    containersForMyBag.size,
)

const contentsForMyBag = findBagContents(myBag, rules)
const totalBagsInMyBag = contentsForMyBag.reduce(
    (total, [countForOneBagType, _]) => total + countForOneBagType,
    0,
)
console.log(`Number of bags that must be inside ${myBag}:`, totalBagsInMyBag)
